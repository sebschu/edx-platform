...
from open_ended_grading.utils import (STAFF_ERROR_MESSAGE, STUDENT_ERROR_MESSAGE,
                                      StudentProblemList, controller_qs, generate_problem_url)
...
def take_action_on_flags(request, course_id):
    ...
    for key in required:
        if key not in request.POST:
            error_message = 'Missing key {0} from submission.  Please reload and try again.'.format(key)
            response = {
                'success': False,
    ...
    try:
        ...
        log.exception(
            ...
            "submission_id: {0}, action_type: {1}, grader_id: {2}".format(
